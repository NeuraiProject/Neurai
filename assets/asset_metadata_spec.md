## Neurai Metadata Specification

Additional fields may be added, but will be ignored by Neurai.
...
```
{
   ...
   "contract_signature": "<Neurai signed contract_hash>",

   "contract_address": "NXXXXXXXXXXXXXXXXXXXXXXXXXXXX",

   "symbol": "Orange",

   "name": "Orange juice",
   
   "issuer": "Orange juice, Inc.",

   "description": "This coin is worth one Orange.",

   "description_mime": "text/x-markdown; charset=UTF-8",
   
   "keywords": "Orange, Orange Stand, Juice",

   "type": "Points",

   "website_url": "https://orangegrow.com/redemption_instructions",
   ...
   "contact_email": "jo_doo@gmail.com",

   "contact_address": "Orange Place|221B Baker Street|London",

   "contact_phone": "788-562-1567",

   "forsale": true,

   "forsale_price": "5000 XNA",
   
   "domain": "orangesite.com",
...
**forsale_price** - To give buyers an idea of the cost to own and admin the asset token.   Price followed by a space, followed by the currency.  Examples: "10000 XNA" or "0.3 BTC" or "50000 USD"  This might be parsed by token broker websites.

**domain** - A root domain for the project (if applicable).  Setting the TXT record for xna.<domain> to a signed message of the token name -- signed by the issuer address.  This could be verified by clients to ensure the token and domain go together.  Example:  Set TXT record for xna.bitactivate.com to the signature of the message "BITACTIVATE".  Any client or individual can verify the issuer address, message "BITACTIVATE" which is the token/asset name, and valid signature in the TXT record for xna.bitactivate.com and return true/false.

**restricted** - Designate the token as being restricted.  One example is "rule144" which means sale may be restricted because of the type of token and the exemption used for issuance.  Other restrictions types can be used here as a signal to explorers, exchanges, or token brokers.  No enforcement of restrictions is built into the Neurai protocol. 
