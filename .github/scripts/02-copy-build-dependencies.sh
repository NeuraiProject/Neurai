...
if [[ ${GITHUB_REF} =~ "release" || ${FORCEBUILDDEPS} = "1" ]]; then
    ...
    cd /tmp
    curl -O https://neurai-build-resources.s3.amazonaws.com/${OS}/neurai-${OS}-dependencies.tar.gz
    curl -O https://neurai-build-resources.s3.amazonaws.com/${OS}/SHASUM
    if [[ $(sha256sum -c /tmp/SHASUM) ]]; then
        cd ${GITHUB_WORKSPACE}/depends
        tar zxvf /tmp/neurai-${OS}-dependencies.tar.gz
    else
