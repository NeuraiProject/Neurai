...
`test/functional/test_framework/test_framework.py:NeuraiTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/neurai_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and neuraid
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_neuraid = config["components"].getboolean("ENABLE_NEURAID")
    if not (enable_wallet and enable_cli and enable_neuraid):
        print("No functional tests to run. Wallet, utils, and neuraid must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if neuraid is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "neuraid"]) is not None:
            print("%sWARNING!%s There is already a neuraid process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "NEURAID" not in os.environ:
        os.environ["NEURAID"] = build_dir + '/src/neuraid' + exeext
        os.environ["NEURAICLI"] = build_dir + '/src/neurai-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `neurai-cli help` (`rpc_interface.txt`).
