...
The Neurai Core daemon can be configured to act as a trusted "border
router", implementing the neurai wire protocol and relay, making
...
The ZeroMQ feature in Neurai Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building neuraid:
...
For instance:
    ...
    $ neuraid -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/neuraid.tx.raw
...
These options can also be provided in neurai.conf.
...
From the perspective of neuraid, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into neuraid directly. Furthermore, no information is
...
using. Neuraid appends an up-counting sequence number to each
