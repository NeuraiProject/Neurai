...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for XNA
    ...
    if (p_rewardSnapshot.strDistributionAsset != "XNA") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is XNA with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using XNA differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "XNA") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Neurai address (already validated during ownership snapshot creation)
