...
const std::string NeuraiGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString NeuraiGUI::DEFAULT_WALLET = "~Default";
...
NeuraiGUI::NeuraiGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Neurai addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Neurai addresses"));
    ...
    openAction->setStatusTip(tr("Open a neurai: URI or payment request"));
...
void NeuraiGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        if(IconsOnly) {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/xnatext")));
        }
        else {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/neuraitext")));
        }
...
//        m_toolbar->addAction(votingAction);
        ...
        QVBoxLayout* neuraiLabelLayout = new QVBoxLayout(toolbarWidget);
        neuraiLabelLayout->addWidget(labelToolbar);
        neuraiLabelLayout->addWidget(m_toolbar);
        neuraiLabelLayout->setDirection(QBoxLayout::TopToBottom);
        neuraiLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Neurai Market Price"));
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/NeuraiProject/Neurai/releases";
...
void NeuraiGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/xnatext")));
        m_toolbar->setMaximumWidth(65);
...
void NeuraiGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool NeuraiGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool NeuraiGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void NeuraiGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void NeuraiGUI::setWalletActionsEnabled(bool enabled)
{
...
void NeuraiGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void NeuraiGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void NeuraiGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void NeuraiGUI::showDebugWindow()
{
...
void NeuraiGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void NeuraiGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void NeuraiGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void NeuraiGUI::openClicked()
{
...
void NeuraiGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void NeuraiGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void NeuraiGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void NeuraiGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void NeuraiGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void NeuraiGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** XNA START */
void NeuraiGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void NeuraiGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void NeuraiGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void NeuraiGUI::gotoRestrictedAssetsPage()
{
...
void NeuraiGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Neurai network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void NeuraiGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void NeuraiGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void NeuraiGUI::updateHeadersSyncProgressLabel()
{
...
void NeuraiGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void NeuraiGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Neurai"); // default title
...
void NeuraiGUI::changeEvent(QEvent *e)
{
...
void NeuraiGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void NeuraiGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void NeuraiGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "XNA")
        msg += tr("Amount: %1\n").arg(NeuraiUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(NeuraiUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void NeuraiGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void NeuraiGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool NeuraiGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool NeuraiGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void NeuraiGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void NeuraiGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void NeuraiGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void NeuraiGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void NeuraiGUI::detectShutdown()
{
...
void NeuraiGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void NeuraiGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void NeuraiGUI::showModalOverlay()
{
...
static bool ThreadSafeMnemonic(NeuraiGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void NeuraiGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<NeuraiUnits::Unit> units = NeuraiUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const NeuraiUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(NeuraiUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(NeuraiUnits::name(unit)));
    #endif
    }
...
void UnitDisplayStatusBarControl::createContextMenu(const PlatformStyle *platformStyle)
{
    ...
    {
        QAction *menuAction = new QAction(QString(NeuraiUnits::name(u)), this);
...
{
    setText(NeuraiUnits::name(newUnits));
}
...
void NeuraiGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void NeuraiGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "NeuraiGUI::onPriceUnitChange: " + QString::number(newIndex);
...
void NeuraiGUI::getLatestVersion()
{
