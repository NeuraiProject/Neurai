...
{
    GUIUtil::setClipboard(NeuraiUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        nBytes = GUIUtil::EstimateControlVBytes(nInputWeight, nQuantity, nWitnessInputs,
            CoinControlDialog::payAmounts.empty() ? 2 : CoinControlDialog::payAmounts.size() + 1);
    ...
    int nDisplayUnit = NeuraiUnits::XNA;
    ...
    l2->setText(NeuraiUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(NeuraiUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(NeuraiUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(NeuraiUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
    ...
    for (const std::pair<QString, std::vector<COutput>>& coins : mapCoins) {
        ...
        for (const COutput& out : coins.second) {
            ...
            if(ExtractDestination(out.tx->tx->vout[out.i].scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show neurai address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, NeuraiUnits::format(nDisplayUnit, out.tx->tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, NeuraiUnits::format(nDisplayUnit, nSum));
