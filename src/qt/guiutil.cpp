...
#include "neuraiaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Neurai address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new NeuraiAddressEntryValidator(parent));
    widget->setCheckValidator(new NeuraiAddressCheckValidator(parent));
}
...
bool parseNeuraiURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no neurai: URI
    if(!uri.isValid() || uri.scheme() != QString("neurai"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!NeuraiUnits::parse(NeuraiUnits::XNA, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseNeuraiURI(QString uri, SendCoinsRecipient *out)
{
    // Convert neurai:// to neurai:
    //
    //    Cannot handle this later, because neurai:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("neurai://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "neurai:");
    }
    QUrl uriInstance(uri);
    return parseNeuraiURI(uriInstance, out);
}

QString formatNeuraiURI(const SendCoinsRecipient &info)
{
    QString ret = QString("neurai:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(NeuraiUnits::format(NeuraiUnits::XNA, info.amount, false, NeuraiUnits::separatorNever));
        paramCount++;
    }
...
bool openNeuraiConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(NEURAI_CONF_FILENAME);
    ...
    /* Open neurai.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Neurai.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Neurai (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Neurai (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Neurai*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "neurai.desktop";
    return GetAutostartDir() / strprintf("neurai-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a neurai.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Neurai\n";
        else
            optionFile << strprintf("Name=Neurai (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the neurai app
...
bool GetStartOnSystemStartup()
{
    CFURLRef neuraiAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (neuraiAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, neuraiAppUrl);

    CFRelease(neuraiAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add neurai app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, neuraiAppUrl, nullptr, nullptr);
    }
