...
const char *const NEURAI_CONF_FILENAME = "neurai.conf";
const char *const NEURAI_PID_FILENAME = "neuraid.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Neurai
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Neurai
    // Mac: ~/Library/Application Support/Neurai
    // Unix: ~/.neurai
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Neurai";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Neurai";
#else
    // Unix
    return pathRet / ".neurai";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string &confPath)
{
    ...
    if (!streamConfig.good())
        return; // No neurai.conf file is OK

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override neurai.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", NEURAI_PID_FILENAME));
