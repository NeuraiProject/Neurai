...
public:
	...
	checkpointData = (CCheckpointData) {
        ...
        // Must match origin/main (546780b): these rules are already live on mainnet.
...
public:
    CTestNetParams() {
        ...
        // KAWPOW header-height rule (origin/main d741868): enforced from height 0 for
        // parity with mainnet's code path, but inert here: testnet is SHA256d
        // (nKAWPOWActivationTime = 0xFFFFFFFF), so no header ever takes the KAWPOW
        // checkpoint shortcut the rule protects.
        consensus.nKAWPOWHeaderHeightCheckActivation = 0;
        // Asset transfer overflow/range enforcement (origin/main 9568f3b) active from
        // height 0 (parity with regtest). The rule applies retroactively on the live
        // testnet chain, so before setting 0 the whole history was replayed with the
        // rule enforced: full -reindex + sync to tip 346486
        // (000022ada50f5b1414eacbb832cdc7bfafded59769beae95ceee1f101c96490a, 2026-08-19)
        // with zero asset-amount rejections. See
        // NIP/190826-port-main-kawpow-height-asset-overflow.md, 3.2.1 / 7.2.
...
public:
    CRegTestParams() {
        ...
        // KAWPOW header-height rule (origin/main d741868): enforced from height 0, but
        // inert here: regtest is SHA256d (nKAWPOWActivationTime = 0xFFFFFFFF).
        consensus.nKAWPOWHeaderHeightCheckActivation = 0;
        // Asset transfer overflow/range enforcement (origin/main 9568f3b) active from
        // height 0 on regtest: no history to validate, and honest tests never build
        // transfers above MAX_MONEY.
        consensus.nAssetTransferOverflowCheckActivation = 0;
