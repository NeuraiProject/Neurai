...
BOOST_FIXTURE_TEST_SUITE(serialization_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(issue_asset_serialization_test)
    {
        ...
        // Create destination using the network's burn address (valid for current chain params)
        std::string strTestAddress = GetParams().GlobalBurnAddress();
        CTxDestination dest = DecodeDestination(strTestAddress);
