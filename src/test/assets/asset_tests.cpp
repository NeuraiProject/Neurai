...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of NEURAICOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("XNA", type));
        BOOST_CHECK(!IsAssetNameValid("NEURAI", type));
        BOOST_CHECK(!IsAssetNameValid("NEURAICOIN", type));

        //- Versions of NEURAICOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("NEURAI.COIN", type));
        BOOST_CHECK(IsAssetNameValid("NEURAI_COIN", type));
        BOOST_CHECK(IsAssetNameValid("XNASPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERXNA", type));
        BOOST_CHECK(IsAssetNameValid("NEURAISPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDENEURAI", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_NEURAIS", type));
        BOOST_CHECK(IsAssetNameValid("SEXNAOT", type));
