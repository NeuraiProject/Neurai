...
// NOTE: there is no end-to-end integration test (mine a KAWPOW header with a mismatched
// declared height and assert ProcessNewBlockHeaders -> ContextualCheckBlockHeader returns
// "bad-blk-height") because regtest and testnet on this branch are SHA256d chains
// (nKAWPOWActivationTime = 0xFFFFFFFF): no header ever takes the KAWPOW path, so the rule
// is inert there by construction and cannot be exercised through block processing. The
// rule's decision, the exploited mechanism, and the index recovery are covered by the
// unit tests above, which do not depend on chainparams.
