...
{
    // Highest assigned flag: bit 48 (NIP-046). Bit 43 remains reserved; NIP-044 uses 44.
    ...
    BOOST_CHECK_EQUAL(MAX_SCRIPT_VERIFY_FLAGS_BITS, 49);
}
...
BOOST_AUTO_TEST_CASE(no_truncation)
{
    ...
    BOOST_CHECK_EQUAL(all.as_int(), (uint64_t{1} << 49) - 1);
}
