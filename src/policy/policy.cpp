...
bool IsStandard(const CScript& scriptPubKey, txnouttype& whichType, const bool witnessEnabled) {
    ...
    if (whichType == TX_MULTISIG) {
        ...
        return false;
    else if (whichType == TX_RESTRICTED_ASSET_DATA && scriptPubKey.size() > GetMaxOPReturnRelay())
        return false;
...
bool IsStandardTx(const CTransaction& tx, std::string& reason, const bool witnessEnabled)
{
    ...
    // only one hundred OP_NEURAI_ASSET txout is permitted
    if (nAssetDataOut > 100) {
        reason = "tomany-op-xna-asset";
        return false;
    }
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++)
    {
        ...
        if (witnessversion == 0 && witnessprogram.size() == 32) {
            ...
            for (unsigned int j = 0; j < sizeWitnessStack; j++) {
                if (tx.vin[i].scriptWitness.stack[j].size() > maxItemSize)
                    return false;
            }
        }
    }
