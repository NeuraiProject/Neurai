...
BOOST_FIXTURE_TEST_CASE(ecdsa_wallet_hands_out_only_v3, RegtestWalletSetup)
{
    ...
    std::string error;
    {
        // Before activation an ecdsa wallet hands out nothing: no Legacy fallback.
        CStrictAuthScriptContext inactive(false);
        BOOST_CHECK(!wallet.GetNewDestination(false, dest, error));
        ...
        BOOST_CHECK(!wallet.CreateNewChangeAddress(reservekey, dest, error));
