...
class CNeuraiAddressVisitor : public boost::static_visitor<bool>
{
private:
    CNeuraiAddress* addr;

public:
    explicit CNeuraiAddressVisitor(CNeuraiAddress* addrIn) : addr(addrIn) {}
...
bool CNeuraiAddress::Set(const CKeyID& id)
{
    SetData(GetParams().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CNeuraiAddress::Set(const CScriptID& id)
{
    SetData(GetParams().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CNeuraiAddress::Set(const CTxDestination& dest)
{
    return boost::apply_visitor(CNeuraiAddressVisitor(this), dest);
}

bool CNeuraiAddress::IsValid() const
{
    return IsValid(GetParams());
}

bool CNeuraiAddress::IsValid(const CChainParams& params) const
{
...
CTxDestination CNeuraiAddress::Get() const
{
    if (!IsValid())
...
bool CNeuraiAddress::GetIndexKey(uint160& hashBytes, int& type) const
{
    if (!IsValid()) {
        return false;
...
void CNeuraiSecret::SetKey(const CKey& vchSecret)
{
...
bool CNeuraiSecret::SetString(const char* pszSecret)
{
