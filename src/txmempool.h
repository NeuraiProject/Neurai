...
#ifndef NEURAI_TXMEMPOOL_H
#define NEURAI_TXMEMPOOL_H
...
public:
    ...
    uint64_t GetPoseidonWork() const { return poseidonWork; }
    void SetPoseidonWork(uint64_t work) { poseidonWork = work; }
...
public:
    ...
    bool getAddressIndex(std::vector<CDestinationIndexData> &addresses, std::string assetName,
                         std::vector<std::pair<CMempoolAddressDeltaKey, CMempoolAddressDelta> > &results);
    bool getAddressIndex(std::vector<CDestinationIndexData> &addresses,
                         std::vector<std::pair<CMempoolAddressDeltaKey, CMempoolAddressDelta> > &results);
