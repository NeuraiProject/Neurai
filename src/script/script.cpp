...
const char* GetOpName(opcodetype opcode)
{
    ...
    case OP_REFINPUTFIELD          : return "OP_REFINPUTFIELD";
...
{
    // 1 OP_XNA_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_XNA_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_XNA_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_XNA_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_XNA_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
