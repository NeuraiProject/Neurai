// Copyright (c) 2009-2010 Satoshi Nakamoto
// Copyright (c) 2009-2016 The Bitcoin Core developers
// Copyright (c) 2019-2022 The Ravencoin developers
// Copyright (c) 2023 The Neurai developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#ifndef NEURAI_SCRIPT_SCRIPT_ERROR_H
#define NEURAI_SCRIPT_SCRIPT_ERROR_H

typedef enum ScriptError_t
{
    SCRIPT_ERR_OK = 0,
    SCRIPT_ERR_UNKNOWN_ERROR,
    SCRIPT_ERR_BACKEND_FAILURE, // local operational failure, not consensus invalidity
    SCRIPT_ERR_ZK_STACK_SIZE,
    SCRIPT_ERR_ZK_BAD_SIGVERSION,
    SCRIPT_ERR_ZK_BAD_PROFILE,
    SCRIPT_ERR_ZK_INPUT_COUNT,
    SCRIPT_ERR_ZK_PUBLIC_INPUT_SIZE,
    SCRIPT_ERR_ZK_PUBLIC_INPUT_RANGE,
    SCRIPT_ERR_ZK_VK_ENCODING,
    SCRIPT_ERR_ZK_PROOF_ENCODING,
    SCRIPT_ERR_ZK_VERIFY_FAILED,
    SCRIPT_ERR_EVAL_FALSE,
    SCRIPT_ERR_OP_RETURN,

    /* Max sizes */
    SCRIPT_ERR_SCRIPT_SIZE,
    SCRIPT_ERR_PUSH_SIZE,
    SCRIPT_ERR_OP_COUNT,
    SCRIPT_ERR_AUTHSCRIPT_HASH_BUDGET,
    SCRIPT_ERR_STACK_SIZE,
    SCRIPT_ERR_SIG_COUNT,
    SCRIPT_ERR_PUBKEY_COUNT,

    /* Failed verify operations */
    SCRIPT_ERR_VERIFY,
    SCRIPT_ERR_EQUALVERIFY,
    SCRIPT_ERR_CHECKMULTISIGVERIFY,
    SCRIPT_ERR_CHECKSIGVERIFY,
    SCRIPT_ERR_NUMEQUALVERIFY,

    /* Logical/Format/Canonical errors */
    SCRIPT_ERR_BAD_OPCODE,
    SCRIPT_ERR_DISABLED_OPCODE,
    SCRIPT_ERR_INVALID_STACK_OPERATION,
    SCRIPT_ERR_INVALID_ALTSTACK_OPERATION,
    SCRIPT_ERR_UNBALANCED_CONDITIONAL,

    /* CHECKLOCKTIMEVERIFY and CHECKSEQUENCEVERIFY */
    SCRIPT_ERR_NEGATIVE_LOCKTIME,
    SCRIPT_ERR_UNSATISFIED_LOCKTIME,

    /* Malleability */
    SCRIPT_ERR_SIG_HASHTYPE,
    SCRIPT_ERR_SIG_DER,
    SCRIPT_ERR_MINIMALDATA,
    SCRIPT_ERR_SIG_PUSHONLY,
    SCRIPT_ERR_SIG_HIGH_S,
    SCRIPT_ERR_SIG_NULLDUMMY,
    SCRIPT_ERR_PUBKEYTYPE,
    SCRIPT_ERR_CLEANSTACK,
    SCRIPT_ERR_MINIMALIF,
    SCRIPT_ERR_SIG_NULLFAIL,

    /* softfork safeness */
    SCRIPT_ERR_DISCOURAGE_UPGRADABLE_NOPS,
    SCRIPT_ERR_DISCOURAGE_UPGRADABLE_WITNESS_PROGRAM,

    /* segregated witness */
    SCRIPT_ERR_WITNESS_PROGRAM_WRONG_LENGTH,
    SCRIPT_ERR_WITNESS_PROGRAM_WITNESS_EMPTY,
    SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH,
    SCRIPT_ERR_AUTHSCRIPT_TREE_CONTROL,
    SCRIPT_ERR_AUTHSCRIPT_TREE_LEAF_VERSION,
    SCRIPT_ERR_WITNESS_MALLEATED,
    SCRIPT_ERR_WITNESS_MALLEATED_P2SH,
    SCRIPT_ERR_WITNESS_UNEXPECTED,
    SCRIPT_ERR_WITNESS_PUBKEYTYPE,

    /* OP_CHECKTEMPLATEVERIFY (BIP 119) */
    SCRIPT_ERR_CHECKTEMPLATEVERIFY,

    /* OP_CHECKSIGFROMSTACK */
    SCRIPT_ERR_CHECKSIGFROMSTACK,

    /* OP_TXHASH */
    SCRIPT_ERR_TXHASH,

    /* OP_TXFIELD */
    SCRIPT_ERR_TXFIELD,

    /* OP_SPLIT */
    SCRIPT_ERR_SPLIT,

    /* OP_OUTPUTVALUE */
    SCRIPT_ERR_OUTPUTVALUE,

    /* OP_OUTPUTSCRIPT */
    SCRIPT_ERR_OUTPUTSCRIPT,

    /* OP_OUTPUTASSETFIELD */
    SCRIPT_ERR_OUTPUTASSETFIELD,

    /* OP_INPUTASSETFIELD */
    SCRIPT_ERR_INPUTASSETFIELD,

    /* OP_OUTPUTAUTHCOMMITMENT (NIP-023) */
    SCRIPT_ERR_OUTPUTAUTHCOMMITMENT,

    /* OP_OUTPUTAUTHDEST (NIP-041) */
    SCRIPT_ERR_OUTPUTAUTHDEST,
    SCRIPT_ERR_INPUTFIELD,

    /* OP_INPUTVALUE (NIP-024) */
    SCRIPT_ERR_INPUTVALUE,

    /* OP_CHAINCONTEXT (NIP-026) */
    SCRIPT_ERR_CHAINCONTEXT,
    SCRIPT_ERR_CHAINCONTEXT_BAD_SELECTOR,

    /* OP_INPUTCOUNT / OP_OUTPUTCOUNT */
    SCRIPT_ERR_INPUTOUTPUTCOUNT,

    /* 64-bit arithmetic */
    SCRIPT_ERR_MUL_OVERFLOW,
    SCRIPT_ERR_DIV_BY_ZERO,
    SCRIPT_ERR_DIV_OVERFLOW,
    SCRIPT_ERR_MOD_BY_ZERO,
    SCRIPT_ERR_MOD_OVERFLOW,
    SCRIPT_ERR_ADD_OVERFLOW,
    SCRIPT_ERR_SUB_OVERFLOW,
    SCRIPT_ERR_NEGATE_OVERFLOW,

    /* OP_TXLOCKTIME */
    SCRIPT_ERR_TXLOCKTIME,

    /* OP_REFINPUTFIELD */
    SCRIPT_ERR_REFINPUTFIELD,

    /* OP_REFINPUTASSETFIELD */
    SCRIPT_ERR_REFINPUTASSETFIELD,

    /* OP_REFINPUTCOUNT */
    SCRIPT_ERR_REFINPUTCOUNT,

    /* NIP-036: per-script Poseidon-input-byte budget exceeded.
       MAX_POSEIDON_INPUT_BYTES_PER_SCRIPT (script.h) caps the cumulative
       byte length of OP_POSEIDON inputs across a script. */
    SCRIPT_ERR_POSEIDON_BUDGET,
    SCRIPT_ERR_POSEIDON_WORK_BUDGET,

    /* NIP-035: OP_CHECKSIG_ED25519 strict-profile structural rejections.
       These are CONSENSUS errors (the malformed-vs-invalid split per
       §4.4 / §4.5: malformed encodings hard-error, well-formed but
       invalid signatures push 0). Categories map 1:1 to the
       crypto::ed25519::StructuralResult enum. */
    SCRIPT_ERR_ED25519_SIG_SIZE,
    SCRIPT_ERR_ED25519_PUBKEY_SIZE,
    SCRIPT_ERR_ED25519_PUBKEY_ENCODING,
    SCRIPT_ERR_ED25519_SIG_ENCODING,

    /* NIP-039: OP_CHECKSIGADD mandatory PQ-pubkey shape check. Raised when
       a public key with prefix 0x05 has a size other than
       1 + ML_DSA_44_PUBKEY_SIZE. This prevalidation fires INDEPENDENT of
       any encoding flag (STRICTENC / WITNESS_PUBKEYTYPE) — it is the only
       NIP-039-specific signature/pubkey rejection; everything else is
       delegated to the existing OP_CHECKSIG encoding helpers. */
    SCRIPT_ERR_PQ_PUBKEY_SIZE,

    SCRIPT_ERR_ERROR_COUNT
} ScriptError;

#define SCRIPT_ERR_LAST SCRIPT_ERR_ERROR_COUNT

const char* ScriptErrorString(const ScriptError error);

#endif // NEURAI_SCRIPT_SCRIPT_ERROR_H
