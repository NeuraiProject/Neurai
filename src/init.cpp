 ...
 * between neuraid, and neurai-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    FlushWallets();
#endif
    GenerateNeurais(false, 0, GetParams());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), NEURAI_CONF_FILENAME));
    if (mode == HMM_NEURAID)
    {
#if HAVE_DECL_DAEMON
    ...
    // NIP-028: report the legacy (pre-activation) default in the help text.
    strUsage += HelpMessageOpt("-maxreorg=<n>", strprintf(_("Set the Maximum reorg depth (default: %u)"), defaultChainParams->MaxReorganizationDepth(0)));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), NEURAI_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt("-pubkeyindex", strprintf(_("Maintain an index of revealed public keys from spending transactions, used for encryption purposes (default: %u)"), DEFAULT_PUBKEYINDEX));
    ...
    strUsage += HelpMessageOpt("-datacarriersize", strprintf(_("Maximum size of data in data carrier transactions we relay and mine (default: %u mainnet, %u testnet)"), MAX_OP_RETURN_RELAY, MAX_OP_RETURN_RELAY_TESTNET));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/NeuraiProject/Neurai>";
    const std::string URL_WEBSITE = "<https://neurai.org>";

    return CopyrightHolders(strprintf(_("Copyright (C) %i-%i"), 2023, COPYRIGHT_YEAR) + " ") + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Neurai is running in a usable environment with all
...
bool AppInitParameterInteraction()
{
    ...
    // Set default OP_RETURN size based on network (512 for testnet, 83 for mainnet)
    nMaxDatacarrierBytes = gArgs.GetArg("-datacarriersize", GetMaxOPReturnRelay());
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Neurai process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", NEURAI_CONF_FILENAME)).string());
    ...
    GenerateNeurais(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
                ...
                // Check for changed -pubkeyindex state
                if (fPubKeyIndex != gArgs.GetBoolArg("-pubkeyindex", DEFAULT_PUBKEYINDEX)) {
                    strLoadError = _("You need to rebuild the database using -reindex-chainstate to change -pubkeyindex");
                    break;
                }
