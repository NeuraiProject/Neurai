        ...
        if (!change_address.empty()) {
            ...
            if (!IsValidDestination(destination)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                                   std::string("Invalid Change Address: Invalid Neurai address: ") + change_address);
            }
        }
    }
...
UniValue issueunique(const JSONRPCRequest& request)
{
    ...
    if (!changeAddress.empty()) {
        ...
        if (!IsValidDestination(destination)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                               std::string("Invalid Change Address: Invalid Neurai address: ") + changeAddress);
        }
    }
...
UniValue transfer(const JSONRPCRequest& request)
{
    ...
    if (!IsValidDestination(to_dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Neurai address: ") + to_address);
    }
    ...
    std::string xna_change_address = "";
    if (request.params.size() > 5) {
        xna_change_address = request.params[5].get_str();
    }
    ...
    CTxDestination xna_change_dest = DecodeDestination(xna_change_address);
    if (!xna_change_address.empty() && !IsValidDestination(xna_change_dest))
        throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("XNA change address must be a valid address. Invalid address: ") + xna_change_address);
    ...
    CCoinControl ctrl;
    ctrl.destChange = xna_change_dest;
...
UniValue transferfromaddresses(const JSONRPCRequest& request)
{
    ...
    std::string xna_change_address = "";
    if (request.params.size() > 6) {
        xna_change_address = request.params[6].get_str();
    }
...
UniValue listtagsforaddress(const JSONRPCRequest &request)
{
    ...
    CTxDestination dest = DecodeDestination(address);
    if (!IsValidDestination(dest))
        throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("Not valid XNA address: ") + address);
...
UniValue transferqualifier(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() > 3) {
        ...
        CTxDestination change_dest = DecodeDestination(change_address);
        if (!IsValidDestination(change_dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Neurai address: ") + change_address);
        }
    }
