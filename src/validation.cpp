...
#if defined(NDEBUG)
# error "Neurai cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Neurai Signed Message:\n";
...
static script_verify_flags GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& chainparams);
...
void UpdateMempoolForReorg(DisconnectedBlockTransactions &disconnectpool, bool fAddToMempool)
{
    ...
    // Strict AuthScript: the per-tip transition sweep in DisconnectTip cannot see
...
static bool CheckInputsFromMempoolAndCache(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &view, CTxMemPool& pool,
                 ...
                 const std::shared_ptr<std::vector<CTxOut>>& pRefOutputs, ChainContext chainCtx = {}) {
    AssertLockHeld(cs_main);
    ...
    return CheckInputs(tx, state, view, true, flags, cacheSigStore, true, txdata, nullptr, pRefOutputs, chainCtx);
}
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        scriptVerifyFlags = ApplyConsensusOptIns(scriptVerifyFlags, chainparams.GetConsensus(), fStrictAuthScriptActive);
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s XNA additional fees, %d delta bytes\n",
...
void UpdateCoins(const CTransaction& tx, CCoinsViewCache& inputs, CTxUndo &txundo, int nHeight, uint256 blockHash, CAssetsCache* assetCache, std::pair<std::string, CBlockAssetUndo>* undoAssetData)
{
    ...
    if (!tx.IsCoinBase()) {
        ...
        for (const CTxIn &txin : tx.vin) {
            ...
            bool is_spent = inputs.SpendCoin(txin.prevout, &txundo.vprevout.back(), assetCache); /** XNA START */ /* Pass assetCache into function */ /** XNA END */
            assert(is_spent);
        }
    }
    // add outputs
    AddCoins(inputs, tx, nHeight, blockHash, false, assetCache, undoAssetData); /** XNA START */ /* Pass assetCache into function */ /** XNA END */
}
...
static void MempoolCheckStrictAuthScriptTransition(CTxMemPool& pool,
...
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, bool fScriptChecks, script_verify_flags flags, bool cacheSigStore, bool cacheFullScriptStore, PrecomputedTransactionData& txdata, std::vector<CScriptCheck> *pvChecks, std::shared_ptr<std::vector<CTxOut>> pRefOutputs, ChainContext chainCtx, bool* pfUsesChainContext)
{
    if (!tx.IsCoinBase())
    {
        ...
        if (fScriptChecks) {
            ...
            auto flagsInt = flags.as_int();
            // We only use the first 15 bytes of nonce to avoid a second SHA
            // round - giving us 15 + 32 + 8 = 55 bytes (+ 8 + 1 = 64)
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        for (size_t o = 0; o < tx.vout.size(); o++) {
            if (!tx.vout[o].scriptPubKey.IsUnspendable()) {
                ...
                Coin coin;
                bool is_spent = view.SpendCoin(out, &coin, &tempCache); /** XNA START */ /* Pass assetsCache into the SpendCoin function */ /** XNA END */
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                int res = ApplyTxInUndo(std::move(undo), view, out, assetsCache); /** XNA START */ /* Pass assetsCache into ApplyTxInUndo function */ /** XNA END */
...
static script_verify_flags GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) {
    AssertLockHeld(cs_main);
    ...
    script_verify_flags flags = fStrictPayToScriptHash ? SCRIPT_VERIFY_P2SH : SCRIPT_VERIFY_NONE;
    ...
    return ApplyConsensusOptIns(flags, consensusparams, consensusparams.IsStrictAuthScriptActive(pindex->nHeight));
}
...
bool static FlushStateToDisk(const CChainParams& chainparams, CValidationState &state, FlushStateMode mode, int nManualPruneHeight) {
    ...
    try {
    {
        ...
        if (fDoFullFlush) {
            ...
            if (!CheckDiskSpace((48 * 2 * 2 * pcoinsTip->GetCacheSize()) + assetDirtyCacheSize * 2)) /** XNA START */ /** XNA END */
                return state.Error("out of disk space");
...
private:
    ...
    CTxMemPool &pool;
    boost::signals2::connection m_notifyEntryRemovedConnection;

public:
    explicit ConnectTrace(CTxMemPool &_pool) : blocksConnected(1), pool(_pool) {
        m_notifyEntryRemovedConnection = pool.NotifyEntryRemoved.connect(boost::bind(&ConnectTrace::NotifyEntryRemoved, this, _1, _2));
    }

    ~ConnectTrace() {
        m_notifyEntryRemovedConnection.disconnect();
    }
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
...

    {
        ...
        LogPrint(BCLog::BENCH, "  - Flush XNA: %.2fms [%.2fs (%.2fms/blk)]\n", (nTime4 - nTime3) * MILLI, nTimeFlush * MICRO, nTimeFlush * MILLI / nBlocksTotal);
    ...
    MempoolCheckAssetMarkerTransition(mempool, pindexNew->nHeight,
                                      pindexNew->nHeight + 1,
                                      chainparams.GetConsensus());
    MempoolCheckStrictAuthScriptTransition(mempool, pindexNew->nHeight,
                                           pindexNew->nHeight + 1,
                                           chainparams.GetConsensus());
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (!ConnectBlock(block, state, &indexDummy, viewNew, chainparams, &assetCache, true)) /** XNA START */ /*Add asset to function */ /** XNA END*/
