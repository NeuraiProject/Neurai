...
dnl Output: If qt version is auto, set neurai_enable_qt to false. Else, exit.
AC_DEFUN([NEURAI_QT_FAIL],[
  if test "x$neurai_qt_want_version" = xauto && test "x$neurai_qt_force" != xyes; then
    if test "x$neurai_enable_qt" != xno; then
      AC_MSG_WARN([$1; neurai-qt frontend will not be built])
    fi
    neurai_enable_qt=no
    neurai_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])

AC_DEFUN([NEURAI_QT_CHECK],[
  if test "x$neurai_enable_qt" != xno && test "x$neurai_qt_want_version" != xno; then
    true
    $1
  else
    true
...
dnl NEURAI_QT_PATH_PROGS([FOO], [foo foo2], [/path/to/search/first], [continue if missing])
...
AC_DEFUN([NEURAI_QT_PATH_PROGS],[
  NEURAI_QT_CHECK([
    ...
    if test "x$$1" = x && test "x$4" != xyes; then
      NEURAI_QT_FAIL([$1 not found])
    fi
  ])
])

dnl Initialize qt input.
dnl This must be called before any other NEURAI_QT* macros to ensure that
...
AC_DEFUN([NEURAI_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [
     ...
     if test "x$neurai_qt_want_version" = xyes; then
       neurai_qt_force=yes
...
dnl
dnl   NEURAI_QT_CONFIGURE([MINIMUM-VERSION])
dnl
dnl Outputs: See _NEURAI_QT_FIND_LIBS
...
AC_DEFUN([NEURAI_QT_CONFIGURE],[
  ...
  dnl _NEURAI_QT_CHECK_STATIC_PLUGIN does a quick link-check and appends the
  ...
  _NEURAI_QT_IS_STATIC
  if test "x$neurai_cv_static_qt" = xyes; then
    _NEURAI_QT_CHECK_STATIC_LIBS
    ...
    if test "x$TARGET_OS" = xwindows; then
      ...
      _NEURAI_QT_CHECK_STATIC_PLUGIN([QWindowsIntegrationPlugin], [-lqwindows])
      ...
      _NEURAI_QT_CHECK_STATIC_PLUGIN([QCocoaIntegrationPlugin], [-lqcocoa])
  ...
  NEURAI_QT_CHECK([
    neurai_enable_qt=yes
    neurai_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      neurai_enable_qt_test=no
    fi
    neurai_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      neurai_enable_qt_dbus=yes
    fi
  ...
  if test x$neurai_enable_qt = xyes; then
    AC_MSG_RESULT([$neurai_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$neurai_enable_qt])
  fi
...
dnl Output: neurai_cv_static_qt=yes|no
AC_DEFUN([_NEURAI_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, neurai_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [neurai_cv_static_qt=yes],
      [neurai_cv_static_qt=no])
    ])
])

dnl Internal. Check if the link-requirements for a static plugin are met.
dnl
dnl _NEURAI_QT_CHECK_STATIC_PLUGIN(PLUGIN, LIBRARIES)
dnl --------------------------------------------------
dnl
dnl Requires: INCLUDES and LIBS must be populated as necessary.
...
AC_DEFUN([_NEURAI_QT_CHECK_STATIC_PLUGIN], [
  ...
  AC_LINK_IFELSE([AC_LANG_PROGRAM([[
    ...
    [AC_MSG_RESULT([no]); NEURAI_QT_FAIL([$1 not found.])])
  LIBS="$CHECK_STATIC_PLUGINS_TEMP_LIBS"
])

dnl Internal. Check Qt static libs with PKG_CHECK_MODULES.
dnl
dnl _NEURAI_QT_CHECK_STATIC_LIBS
dnl -----------------------------
dnl
...
AC_DEFUN([_NEURAI_QT_FIND_LIBS],[
  NEURAI_QT_CHECK([
    PKG_CHECK_MODULES([QT_CORE], [${qt_lib_prefix}Core${qt_lib_suffix} $qt_version], [],
                      [NEURAI_QT_FAIL([${qt_lib_prefix}Core${qt_lib_suffix} $qt_version not found])])
  ])
  NEURAI_QT_CHECK([
    PKG_CHECK_MODULES([QT_GUI], [${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version], [],
                      [NEURAI_QT_FAIL([${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version not found])])
  ])
  NEURAI_QT_CHECK([
    PKG_CHECK_MODULES([QT_WIDGETS], [${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version], [],
                      [NEURAI_QT_FAIL([${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version not found])])
  ])
  NEURAI_QT_CHECK([
    PKG_CHECK_MODULES([QT_NETWORK], [${qt_lib_prefix}Network${qt_lib_suffix} $qt_version], [],
                      [NEURAI_QT_FAIL([${qt_lib_prefix}Network${qt_lib_suffix} $qt_version not found])])
  ])
