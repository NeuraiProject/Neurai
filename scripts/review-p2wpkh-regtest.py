#!/usr/bin/env python3
"""P2SH-P2WPKH wallet signing and canonical redeemScript in full blocks."""
import argparse
import hashlib
...
def main():
    ...
    directory = Path(tempfile.mkdtemp(prefix='p2wpkh-regtest-'))
    ...
    try:
        ...
        pubkey = bytes.fromhex(keys.rpc('validateaddress', address)['pubkey'])
        redeems = [b'\x00\x14' + hashlib.new('ripemd160', a.sha256(pubkey)).digest()]
        secrets = [keys.rpc('dumpprivkey', address)]
        if args.keys_first:
        ...
        if not args.keys_first:
            for secret in secrets:
                source.rpc('importprivkey', secret, '', False)
        raw = struct.pack('<I', 2) + b'\x00' + h.compact(len(outputs)) + b''.join(b.output(1_000_000_000, p) for p in outputs) + bytes(4)
        ...
        for version, (redeem, p2sh) in enumerate(zip(redeems, outputs)):
            ...
            if True:
                ...
                check(label + '/keyhash_witness', len(witness) == 2 and witness[1] == pubkey and len(witness[0]) > 0, [len(x) for x in witness])
