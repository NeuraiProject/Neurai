#!/usr/bin/env python3
"""Real v1/v2/v3 signatures for six sighash modes, P2SH and v3 references."""
import argparse
...
def signhash(utxo, outputs, refs, ht, version, auth):
    ...
    prev = h.outpoint(*utxo)
    sequence = b'\xff' * 4
    mode = ht & 31
    preimage = struct.pack('<I', txversion)
    preimage += bytes(32) if ht & 128 else b.hash256(prev)
    ...
    preimage += bytes(32) if mode == 2 else b.hash256(outputs[0] if mode == 3 else b''.join(outputs))
    if refs:
...
def transaction(utxo, sigscript, stack, outputs, refs, sequence=0xffffffff):
    inputs = b'\x01' + h.outpoint(*utxo) + h.compact(len(sigscript)) + sigscript + struct.pack('<I', sequence)
...
def main():
    ...
    directory = Path(tempfile.mkdtemp(prefix='sighash-modes-'))
    ...
    try:
        ...
        outputs = [b.output(900_000_000, destination), b.output(50_000_000, destination)]
        keys = {}
        ...
        refouts = [(funding, n) for n in available[refspk.hex()]]
        validators = []
        for par in (1, 2):
        ...
        for label, version, auth, family, ht, withrefs, spk, sigscript in contracts:
            utxo = funding, available[spk.hex()].popleft()
            ...
            changed_outputs = list(outputs)
            change_index = 1 if ht & 31 == 3 else 0
            changed_outputs[change_index] = b.output((50_000_000 if change_index else 900_000_000) + 1, destination)
            mutation = transaction(utxo, sigscript, stack, changed_outputs, refs)
            if ht & 31 == 1:
                negatives.append(('output', mutation))
                good = original
            else:
                accepted = source.rpc('testmempoolaccept', [mutation[1].hex()])[0]
            ...
            for par, node in validators:
                ...
                check(label + f'/par{par}/state', node.rpc('gettxout', *utxo) is None and node.rpc('gettxout', txid, 0) is not None, txid)
            if refs:
