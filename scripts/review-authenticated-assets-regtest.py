...
def commitment(script):
    tag = sha256(b'NeuraiAuthScript')
    descriptor = bytes([AUTH]) + (hashlib.new('ripemd160', sha256(PUB)).digest() if AUTH else b'')
    return sha256(tag + tag + b'\x01' + descriptor + sha256(script))


def contract_address(node, digest):
    native = b'\x51\x20' + digest
    return node.rpc('decodescript', native.hex())['p2sh'] if WRAPPED else bech32m('tnq', 1, digest)


def contract_output(digest):
    native = b'\x51\x20' + digest
    return b'\xa9\x14' + hashlib.new('ripemd160', sha256(native)).digest() + b'\x87' if WRAPPED else native


def raw_transaction(inputs, outputs, refs, witnesses):
    contract = witnesses[0][-1]
    prevouts = b''.join(outpoint(*p) for p in inputs)
    serialized_outputs = b''.join(struct.pack('<Q', value) + compact(len(script)) + script for value, script in outputs)
    witnesses = [list(w) for w in witnesses]
    if AUTH:
        double = lambda x: sha256(sha256(x))
        preimage = (struct.pack('<I', 3) + double(prevouts) + double(b'\xff' * (4 * len(inputs))) +
                    outpoint(*inputs[0]) + compact(len(contract)) + contract + struct.pack('<Q', COIN) + b'\xff' * 4 +
                    double(serialized_outputs) + double(b''.join(outpoint(*p) for p in refs)) + bytes(4) + bytes([AUTH]) + struct.pack('<I', 1))
        signature = bytes.fromhex(subprocess.check_output([str(SIGNER), 'sign', 'pq' if AUTH == 1 else 'ecdsa'],
                                  input=SECRET + '\n' + double(preimage).hex() + '\n', text=True).strip())
        witnesses[0] = [bytes([AUTH]), signature, PUB, contract]
    raw = struct.pack('<I', 3) + b'\x00\x01' + compact(len(inputs))
    for i, (txid, index) in enumerate(inputs):
        script_sig = push(b'\x51\x20' + commitment(contract)) if WRAPPED and i == 0 else b''
        raw += outpoint(txid, index) + compact(len(script_sig)) + script_sig + b'\xff' * 4
    raw += compact(len(outputs)) + serialized_outputs
...
def main():
    ...
    global AUTH, WRAPPED, PUB, SECRET, SIGNER
    AUTH, WRAPPED, SIGNER = args.auth, args.wrapped, args.signer
    if AUTH:
        pub, SECRET = subprocess.check_output([str(SIGNER), 'keygen', 'pq' if AUTH == 1 else 'ecdsa'], text=True).splitlines()
    ...
    try:
        ...
        for version in (2, 3):
            ...
            tag = sha256(b'NeuraiAuthScript')
            contract_hash = commitment(contract)
            contract_spk = contract_output(contract_hash)
            funding = confirmed(f'v{version}/fund_contract', node.rpc('sendtoaddress', contract_address(node, contract_hash), 1))
        ...
        def inspect_asset(label, tx, version, marker, name, amount, fields, missing):
            ...
            def contract_spend(mode, expected, unavailable=None):
                ...
                tag = sha256(b'NeuraiAuthScript')
                contract_hash = commitment(contract)
                spk = contract_output(contract_hash)
                ...
                funding = confirmed(label + '/' + mode + '/fund_' + fund_case,
                                    node.rpc('sendtoaddress', contract_address(node, contract_hash), 1))
