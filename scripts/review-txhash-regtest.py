...
def field_hash(mask, version, locktime, prevouts, sequences, outputs, index):
    ...
    return double_sha(b''.join(field for bit, field in enumerate(fields) if mask & (1 << bit)))
...
def vectors():
    ...
    for row in _ctv.vectors():
        ...
        hashes = [field_hash(mask, 3, 123, prevouts, [0xfffffffe, 0xfffffffd],
                             [(100000, script), (200000, b'\x51')], 1) for mask in range(1, 256)]
    ...
    try:
        ...
        for family, output in enumerate(destinations):
            for mask in (1, 16, 17):
                ...
                digest = field_hash(mask, 3, 0, [bytes(36)], [0xffffffff], [(99000000, output)], 0)
                script = h.push(bytes([mask])) + b'\xb5' + h.push(digest) + b'\x87'
                tag = _ctv.sha(b'NeuraiAuthScript')
                commitment = _ctv.sha(tag + tag + b'\x01\x00' + _ctv.sha(script))
                utxo = fund(_ctv.bech32m('tnq', 1, commitment), b'\x51\x20' + commitment)
                ...
                check(label + '/references_not_committed', result.get('allowed') == 1, result)
