...
Only B3/vault/deposit execute; other MAST leaves contain unexecuted DEMO VKs.
"""
...
leaves=c.all_leaves();commitment,paths=c.mast(leaves);old=bytes(108)+b'\x01'
vault=c.auxiliary('vault',commitment).bytes();deposit=c.auxiliary('deposit',commitment).bytes()
data={'scope':'B3 and non-ZK auxiliaries only; other committed leaves/VKs are DEMO, not verified',
 ...
 'leaf':leaves['B3'].bytes().hex(),'control':(b'\x01'+b''.join(paths['B3'])).hex(),
 'opcodes':leaves['B3'].nops(),'old':old.hex(),'state_hash':pose.sponge(old),
 'vault':vault.hex(),'vault_program':c.commit(vault).hex(),
 'deposit':deposit.hex(),'deposit_program':c.commit(deposit).hex()}
f=ROOT/'src/test/data/nip043_b3_fixture.json'
...
print(json.dumps({'fixture':'match','opcodes':data['opcodes'],'leaf_bytes':len(bytes.fromhex(data['leaf']))}))
