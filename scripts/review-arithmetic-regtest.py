...
def cases():
    # All contracts require exactly 0.1 XNA fee from a real 1 XNA input.
    fee = b'\x00\xd6\x00\xcc\x94' + h.push(number(10_000_000)) + b'\x9d'
    # A witness operand is combined with the actual output value (90M sat).
    ...
    for name, opcode, operand, bad, error in [
        ...
        ('ADD', 0x93, 2**63-1-90_000_000, 2**63-1,
    ...
    try:
        ...
        for label, script, arguments, output, program, family, bad_arguments, failure in contracts:
            ...
            negatives = [
                ...
                ('wrong_amount', transaction(utxo, script, arguments, output, 89_999_999),
