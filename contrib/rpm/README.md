...
The RPM spec file provided here is for Neurai-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://neurai.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Neurai git repository](https://github.com/NeuraiProject/Neurai) but are not part of
...
Currently only `Source10` is used, for the example `neurai.conf` file.
...
in the neurai git repository. At present only an SVG version of the Neurai
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the neurai
...
needed when 0.12.1 is released, a proper fix is already in the Neurai git
master branch.
...
idea to build Neurai Core against a static version of that library compiled
...
The `neurai.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `neurai-server` package. At this point, most RPM based
...
The `neurai-server` package should have SELinux support. How to properly do
...
Neurai-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/neurai/). He has been
packaging Neurai for Fedora at least since 2012.
...
4. I renamed the `neurai` package that contains the Qt GUI to `neurai-core` as
that appears to be how the general population refers to it, in contrast to
`neurai-xt` or `neurai-classic`. I wanted to make sure the general population
